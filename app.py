...
import json
import socket
from motor_controller import move, start_swim, stop_swim, current_state, swim_config, load_current_state, get_motor_status
...
@app.route('/config')
def config():
    return render_template('config.html', swim_config=swim_config, current_height=current_state['current_height'], hostname=hostname)
